...
#ifdef EMBREE_TRACING
static void BM_EMBREE_TRAVERSE_WORST_Sphere(benchmark::State &state) {
  using embreeVec3 =
...
static void BM_bvh_TRAVERSE_WORST_Sphere_SweepSAH(benchmark::State &state) {
  ...
  for (auto _ : state) {
    for (auto &dir : os->vertices) {
      ...
      Ray ray(Vector3(0.0, 0.0, 0.0),         // origin
...
BENCHMARK_TEMPLATE(BM_bvh_TRAVERSE_WORST_Sphere_SweepSAH, float)->DenseRange(2, 9, 1)->Unit(benchmark::kMillisecond);
BENCHMARK_TEMPLATE(BM_bvh_TRAVERSE_WORST_Sphere_SweepSAH, double)->DenseRange(2, 9, 1)->Unit(benchmark::kMillisecond);
...
BENCHMARK_TEMPLATE(BM_bvh_TRAVERSE_WORST_Sphere_BinnedSAH, float)->DenseRange(2, 9, 1)->Unit(benchmark::kMillisecond);
BENCHMARK_TEMPLATE(BM_bvh_TRAVERSE_WORST_Sphere_BinnedSAH, double)->DenseRange(2, 9, 1)->Unit(benchmark::kMillisecond);
