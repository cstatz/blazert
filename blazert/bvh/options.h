...
public:
  ...
  explicit BVHBuildOptions(const unsigned int min_leaf_primitives = 4,
                           const unsigned int max_tree_depth = BLAZERT_MAX_TREE_DEPTH,
                           const unsigned int bin_size = 64)
      :
        min_leaf_primitives(min_leaf_primitives),
        max_tree_depth(max_tree_depth),  /// Does not impact build time significantly, but impacts memory demand and traversal time.
