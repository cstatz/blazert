...
inline bool IntersectRayAABB(T *tmin, T *tmax, const T min_t, const T max_t, const Vec3r<T> &bmin, const Vec3r<T> &bmax,
  ...
  T tmin_x = (min_x - ray_org[0]) * ray_inv_dir[0];
  T tmax_x = (max_x - ray_org[0]) * ray_inv_dir[0] * (1 + 4 * std::numeric_limits<T>::epsilon());
  ...
  if ((tmin_x >= min_t) && (tmax_x < max_t)) {
    ...
    //std::cout << tmin_x << " : " << min_t << ",  "<< tmax_x << " : " << max_t << std::endl;
    *tmin = tmin_x;
    *tmax = tmax_x;
    return true;
    }
