...
public:  // Public methods
  ...
  bool Traverse(const Ray<T> &ray, const I &intersector, H &isect, const BVHTraceOptions<T> &options=BVHTraceOptions<T>()) const;
   ...
   */
  template<class I>
  bool ListNodeIntersections(const Ray<T> &ray, unsigned int max_intersections, const I &intersector, StackVector<NodeHit<T>, 128> &hits) const;
...
private:  // Private methods
  ...
  bool TestLeafNodeIntersections(
      const BVHNode<T> &node, const Ray<T> &ray, const int max_intersections, const I &intersector,
  ...
  template<class P, class Pred>
  unsigned int BuildShallowTree(std::vector<BVHNode<T>> &out_nodes, unsigned int left_idx, unsigned int right_idx,
                                unsigned int depth, unsigned int max_shallow_depth, const P &p, const Pred &pred);

  // TODO: Where is this needed?
  // template<class P, class Pred>
  // unsigned int BuildShallowTree(std::vector<T> &out_nodes, unsigned int left_idx, unsigned int right_idx, unsigned int depth,
...
template<typename T>
template<class P, class Pred>
unsigned int BVHAccel<T>::BuildTree(BVHBuildStatistics &out_stat, std::vector<BVHNode<T>> &out_nodes, unsigned int left_idx,
...
template<typename T>
template<class Prim, class Pred>
bool BVHAccel<T>::Build(const Prim &p, const Pred &pred, const BVHBuildOptions<T> &options) {

  options_ = options;
  stats_ = BVHBuildStatistics();
  nodes_.clear();
  bboxes_.clear();
#if defined(BLAZERT_PARALLEL_BUILD)
...
#ifdef BLAZERT_PARALLEL_BUILD_THREADS
  {
    size_t num_threads = std::min(BLAZERT_MAX_THREADS, std::max(1, std::thread::hardware_concurrency()));
    ...
    size_t ndiv = p.size() / num_threads;

    for (size_t t = 0; t < num_threads; t++) {
      workers.emplace_back(std::thread([&, t]() {
        size_t si = t * ndiv;
        size_t ei = (t == (num_threads - 1)) ? n : std::min((t + 1) * ndiv, p.size());
...
#ifdef BLAZERT_PARALLEL_BUILD
#ifdef BLAZERT_PARALLEL_BUILD_THREAD
...
// 3. Build tree
//
#ifndef BLAZERT_PARALLEL_BUILD
  BuildTree(stats_, nodes_, 0, n, 0, p, pred);
#else
  if (n <= options.min_prmitives_for_parallel_build) {BuildTree(stats_, nodes_, 0, n, 0, p, pred);}
#ifdef BLAZERT_ENABLE_PARALLEL_BUILD_THREADS
  else {

    BuildShallowTree(nodes_, 0, n, 0, options.shallow_depth, p, pred);
    ...
    for (size_t ii = 0; ii < local_nodes.size(); ii++) {
      ...
      nodes_.insert(nodes_.end(), local_nodes[ii].begin() + 1, local_nodes[ii].end());
    }

    // Join statistics
    for (size_t ii = 0; ii < local_nodes.size(); ii++) {
      stats_.max_tree_depth = std::max(stats_.max_tree_depth, local_stats[ii].max_tree_depth);
...
template<typename T>
void BVHAccel<T>::print_bvh_as_json() const {

  // Output as JSON
  std::cout << "{" << std::endl; //JSON
  std::cout << R"("indices": [ )" << std::endl;

  for (size_t i = 0; i < indices_.size(); i++) {
    std::cout << int(indices_[i]);
    if (i < indices_.size()-1) { std::cout << ", ";}
  }

  std::cout << "], " << std::endl;
  std::cout << R"("nodes": [ )" << std::endl;

  for (size_t i = 0; i < nodes_.size(); i++) {
    std::cout << "{";
    std::cout << R"("bmin": [)";
    for (int kk=0; kk<3; kk++) {
      std::cout << nodes_[i].bmin[kk];
      if (kk < 2) { std::cout << ", ";}
    }
    std::cout << R"(], "bmax": [)";
    for (int kk=0; kk<3; kk++) {
      std::cout << nodes_[i].bmax[kk];
      if (kk < 2) { std::cout << ", ";}
    }
    std::cout << "]}";
    if (i < nodes_.size()-1) { std::cout << ", ";}
    else {std::cout << "]";}
    std::cout << std::endl;
  }
...
bool BVHAccel<T>::Traverse(const Ray<T> &ray, const I &intersector, H &isect,
                           const BVHTraceOptions<T> &options) const {
  ...
  const Vec3r<T> ray_inv_dir = 1./ray.dir; // Check this is safe inv ..
  const Vec3r<T> ray_dir = ray.dir;
  const Vec3r<T> ray_org = ray.org;
...
bool BVHAccel<T>::ListNodeIntersections(
  ...
  const Vec3r<T> ray_inv_dir = 1./ray.dir;
  const Vec3r<T> ray_dir = ray.dir;
  const Vec3r<T> ray_org = ray.org;
  ...
  if (!isect_pq.empty()) {
    ...
    // Store intersection in reverse order (make it front most order)
    const size_t n = isect_pq.size();
    hits.resize(n);
